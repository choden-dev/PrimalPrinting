	...
	buildConfig({
		...
		db: mongooseAdapter({
			...
			connectOptions: {
				...
				// Pin a warm socket pool so requests after a quiet gap reuse an
				// authenticated connection instead of paying a fresh TLS + SCRAM
				// handshake (the driver defaults to minPoolSize 0, tearing down
				// idle sockets). maxIdleTimeMS: 0 keeps pooled sockets alive.
				minPoolSize: 2,
				maxPoolSize: 10,
				maxIdleTimeMS: 0,
				// Disable autoIndex in production: rebuilding every collection's
				// indexes on each cold connect makes the first request slow. Create
				// indexes once via `payload migrate`; keep it on in dev so schema
